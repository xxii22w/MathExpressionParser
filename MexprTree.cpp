...
MexprTree::destroy() {
    ...
    this->destory_internal(this->root);
}

void 
MexprTree::destory_internal(MexprNode* root)
{
    Dtype_VARIABLE* dtype_var;
    if(!root)return;

    destory_internal(root->left);
    destory_internal(root->right);
...
// 目的是检查 MexprTree 对象是否代表一个只包含单个变量的操作数的数学表达式树
bool 
...
// 目的是将一个叶节点（leaf_node）替换为一个子树（child_tree），并将叶节点从其父树中移除
bool
